...
**DurationSelect** (`DurationSelect.tsx`) — single-select **ordinal** control, distinct from both patterns above: not a checkbox grid (that implies unordered/multi-select categories), a compact row of 5 pills instead, since duration is a progression like intensity, not a category set. Optional, defaults to "Just happened," never blocks `canSubmit`. Backs the "How long has this been going on?" step; vocabulary lives in `lib/duration.ts`. Also feeds `getActiveStage()`'s severity blend (5% weight, smallest of the three inputs), see `docs/formulas.md`.
