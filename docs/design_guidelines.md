...
- **Diagnosis** (`ScreenAnalysis.tsx`): a tiny `CLINICAL NOTE / EPICRISIS 001` kicker, then a `DIAGNOSIS` eyebrow + condition/code pair (colored `stage.color`, tightened to `items-end`/`leading-tight` so the row doesn't read as oversized empty space), then `WHAT HAPPENED` (cause) and `PRESCRIBED RESPONSE` (subgenre — colored `stage.color`, not a separate accent — + choice text) sections separated by `DIVIDER_COLOR` hairlines, then the citation footnote and the "Get Prescription" CTA. "Epicrisis" is now only that opening kicker — it's no longer the dominant heading. Body copy (cause/choice) sits at `13px`/`leading-[1.45]` on desktop — precise/editorial, not loose.
