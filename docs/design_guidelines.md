...
- **Diagnosis** (`ScreenAnalysis.tsx`): a tiny `CLINICAL NOTE / EPICRISIS 001` kicker, then `INCIDENT SUMMARY` (cause) and `PRESCRIBED RESPONSE` (subgenre — colored `stage.color`, not a separate accent — + choice text) sections separated by `DIVIDER_COLOR` hairlines, then the citation footnote and the "Get Prescription" CTA. There is no separate Diagnosis heading: the stage rail already establishes the diagnosis. Body copy (cause/choice) sits at `13px`/`leading-[1.45]` on desktop — precise/editorial, not loose.
- **Prescription** (`ResultsPanel.tsx`): left-aligned `PRESCRIPTION` heading with a quiet, right-aligned `VIEW RECEIPT` utility action (tight `pb-2` to the divider below it, not a large gap; the wrapper in `ScreenDescent.tsx` also sits closer to the rail above it once there's a result — `mt-5`, not `mt-7` — since the rail and the prescription it produced are related, not two detached blocks). The utility action is muted `TEXT_SECONDARY` and adopts `TREATMENT_ACCENT_TEXT` only on hover; it remains a 44px touch target on mobile. Then a `FOR`/`SEVERITY`/`TREATMENT` key-value block (monospace metadata; labels in `TEXT_SECONDARY`, not `TEXT_TERTIARY` — secondary but legible, not faint; values in `TEXT_PRIMARY` so the block uses one neutral ink treatment). Then the 10-band list: numbers are small (`text-[9px]`, `TEXT_TERTIARY`) *and* rendered at `opacity: 0.6` — pure indexing metadata, not information, so they should recede further than any other text on screen; band names stay at `text-[14px]` bold sans (don't push these larger — they already carry enough weight). The Bandcamp listen icon per row stays at `opacity-40` and only reaches full opacity on hover/focus (`group-hover`/`group-focus-within` on the row) — quiet until interacted with, so it doesn't compete with the band name as a repeated coral dot down the list. Then a `DOSAGE` section with the sig line. No width/centering of its own — inherits `max-w-3xl` from `ScreenDescent`'s root, same as Diagnosis.
