...
At `<=640px`, reflow to two columns and render both trigger and symptom option labels at `13px`, with `26px` checkbox controls. This keeps the two intake checklists visually consistent while leaving the incident prompt at its Safari-safe `16px` size.

**IntensitySlider** — only ever mounts once a category is selected (see Progressive Disclosure below); no "disabled" visual state exists for it. Discrete 0–9 dial + a hidden "ELEVEN" zone past the track's right edge. `StateOfMindPanel` owns a definite `min(280px, calc(100vw - 2rem))` wrapper around it, rather than percentage sizing inside nested centered flex boxes; the slider then pins to that wrapper at ≤640px, so a long Finnish tier label cannot resize the track in Safari. The readout label (`X/10`, or the Finnish tier name for `fi-*` locales) always uses `TEXT_PRIMARY`, never the tier's own ramp color — the bar/dots/handle carry the tier color, the label just needs to stay legible at every tier (some ramp colors, e.g. light green/yellow, are low-contrast for text).
