...
- **Severity** (`stage.color`): the stage rail/header, the Prescription's `SEVERITY` row, **and the subgenre/treatment name on both screens** (Diagnosis's "Prescribed Response" value, Prescription's `TREATMENT` row). One dynamic accent per diagnosis, not two competing ones — an earlier version colored severity and treatment differently (orange vs. terracotta) within the same screen; that read as two unrelated accents rather than one result. Never a new hardcoded hex — always the active `Stage`'s own color, so it stays in sync with the rail/header elsewhere on screen.
...
**Diagnosis and Prescription screens** — no card, no paper background, no border, no shadow. Both are plain page content, same visual register as the landing page, and both share the exact same `max-w-3xl` column and left edge (see Screen width above):
- **Diagnosis** (`ScreenAnalysis.tsx`): a tiny `CLINICAL NOTE / EPICRISIS 001` kicker, then a plain `DIAGNOSIS` eyebrow (no trigger label or "Code {roman}" value next to it anymore — that duplicated the stage rail's own roman numeral above, and was removed), then `INCIDENT SUMMARY` (cause) and `PRESCRIBED RESPONSE` (subgenre — colored `stage.color`, not a separate accent — + choice text) sections separated by `DIVIDER_COLOR` hairlines, then the citation footnote and the "Get Prescription" CTA. "Epicrisis" is now only that opening kicker — it's no longer the dominant heading. Body copy (cause/choice) sits at `13px`/`leading-[1.45]` on desktop — precise/editorial, not loose.
- **Prescription** (`ResultsPanel.tsx`): left-aligned `PRESCRIPTION` heading (tight `pb-2` to the divider below it, not a large gap; the wrapper in `ScreenDescent.tsx` also sits closer to the rail above it once there's a result — `mt-5`, not `mt-7` — since the rail and the prescription it produced are related, not two detached blocks). Then a `FOR`/`SEVERITY`/`TREATMENT` key-value block (monospace metadata; labels in `TEXT_SECONDARY`, not `TEXT_TERTIARY` — secondary but legible, not faint; `SEVERITY` and `TREATMENT` values both colored `stage.color`). Then the 10-band list: numbers are small (`text-[9px]`, `TEXT_TERTIARY`) *and* rendered at `opacity: 0.6` — pure indexing metadata, not information, so they should recede further than any other text on screen; band names stay at `text-[14px]` bold sans (don't push these larger — they already carry enough weight). The Bandcamp listen icon per row stays at `opacity-40` and only reaches full opacity on hover/focus (`group-hover`/`group-focus-within` on the row) — quiet until interacted with, so it doesn't compete with the band name as a repeated coral dot down the list. Then a `DOSAGE` section with the sig line. No width/centering of its own — inherits `max-w-3xl` from `ScreenDescent`'s root, same as Diagnosis.
