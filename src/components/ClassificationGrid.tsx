...
export default function ClassificationGrid({ onSelectionChange, selection, disabled = false }: ClassificationGridProps) {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-3 max-[640px]:grid-cols-2">
        {TRIGGER_TYPES.map((c, index) => {
          ...
          // grid itself reflows from 3 columns (desktop) to 2 (at <=640px) —
          ...
          // `max-[640px]:border-t` case below covers indices that only
          ...
          return (
            <button
              ...
              className={cn(
                ...
                needsMobileOnlyBorder && 'max-[640px]:border-t'
              )}
              style={{
                borderColor: '#e6e2d8',
            ...
            >
              ...
              <span
                className="uppercase whitespace-nowrap max-[640px]:whitespace-normal max-[640px]:!text-[13px]"
                style={{
                  fontSize: '12px',
